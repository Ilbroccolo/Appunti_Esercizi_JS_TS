// enumerazione per i codici che denotano le priorità
enum Code {
    White = 0,
    Green = 1,
    Yellow = 5,
    Red = 10
}

enum Age {
    Adult = 0,
    Elder = 1,
    Child = 2,
    Baby = 5
}

type Urgency = [Code, Age];

function triage(patients: Urgency[]): void {
    patients.sort(function(pazienteA: Urgency, pazienteB: Urgency): number {
        // calcoliamo la priorità sommando i due valori
        let prioritaA: number = pazienteA[0] + pazienteA[1];
        let prioritaB: number = pazienteB[0] + pazienteB[1];

        if (prioritaA !== prioritaB) {
            return prioritaB - prioritaA;
        } else {
            return pazienteB[0] - pazienteA[0];
        }
    });
}
